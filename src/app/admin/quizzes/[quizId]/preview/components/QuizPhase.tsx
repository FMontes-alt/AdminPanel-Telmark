...
import { XCircle, Clock, CheckCircle2, ArrowLeft, ArrowRight, Send } from "lucide-react"
...
export default function QuizPhase({
    ...
    return (
        <div className="h-[calc(100vh-80px)] bg-gradient-to-br from-slate-50 to-blue-50/30 flex flex-col rounded-[32px] overflow-hidden border border-slate-200 shadow-xl">
            {/* Top Bar */}
            <div className="bg-white border-b border-slate-100 px-6 py-3 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    <button onClick={onBack} className="mr-2 text-slate-400 hover:text-slate-700">
                        <XCircle size={20} />
                    </button>
            ...
            {/* Question Content */}
            <div className="flex-1 flex items-center justify-center p-4 py-8 overflow-y-auto no-scrollbar">
                <AnimatePresence mode="wait">
                    ...
                    >
                        {/* Media */}
                        {currentMediaUrl && currentQuestion.mediaType !== "none" && (
                            <div className="rounded-[24px] overflow-hidden border border-slate-200 shadow-sm bg-white relative min-h-[100px]">
                                {resolvingMedia && (
                                ...
                                {currentQuestion.mediaType === "image" ? (
                                    <img src={currentMediaUrl} alt="Pregunta" className="w-full max-h-72 object-contain bg-slate-50" />
                                ) : (
                                    <video src={currentMediaUrl} controls className="w-full max-h-72" />
                                )}
                            </div>
                        )}
            ...
            <div className="bg-white border-t border-slate-100 px-6 py-4 flex items-center justify-between sticky bottom-0">
                ...
                {isLastQuestion ? (
                    ...
                    >
                        <Send size={14} /> {submitting ? "Enviando..." : "Finalizar Cuestionario"}
                    </button>
                ) : (
                    <button
                        onClick={onNext}
