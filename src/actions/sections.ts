...
export async function updateSection(id: string, data: UpdateSectionInput): Promise<ActionResult<typeof sections.$inferSelect>> {
    try {
        await requireAdmin()
        ...
        if (data.imagePath !== undefined && section?.imagePath && section.imagePath !== data.imagePath && !section.imagePath.startsWith('http')) {
            try {
                ...
                log.error("Error eliminando imagen antigua de sección:", error)
            }
        }
        ...
        if (data.config) {
            const config = data.config
            const oldConfig = section?.config as SectionConfig | undefined
...
export async function deleteSection(id: string): Promise<ActionResult> {
    try {
        ...
        try {
            ...
            log.error("Error en limpieza de storage al borrar sección:", error)
